...
**Log Sources:** InteractiveSigninLogs, UnifiedAuditLogs_CL
## Summary
A phishing email led to a device-code authentication compromise. The attacker used the resulting access to create a guest account, then modified two attributes on it — `Country` and `Department` — matching the membership rule of a dynamic group called `Operations`. 58 seconds later, the guest was added to the group automatically by the dynamic-group engine, not by a human actor. The attacker never touched the group directly; they shaped the guest's attributes until the engine did it for them.
## Scope
...
- Tables queried: `MTSummaryMessage_CL`, `SigninLogs`, `UnifiedAuditLogs_CL`
...
![Timeline overview](screenshots/00-timeline-overview.png)

## Investigation

**Phishing email delivery.** Message trace confirms delivery of the lure at 16:48:19, subject "Security code," originating from `185.159.157.42`.
...
**Phishing → device-code sign-in.** Attacker generates a device-code request through an app, tricks the victim into completing it. The resulting sign-in carries the *victim's* identity, not the attacker's.

```kql
SigninLogs
| where UserPrincipalName == "rudolph.emma@compliantsecure.store"
| where TimeGenerated between (datetime(2025-07-11 16:45:00) .. datetime(2025-07-11 17:00:00))
| project TimeGenerated, UserPrincipalName, AuthenticationProtocol, AppDisplayName, ClientAppUsed, IPAddress, ResultType
```

![Sign-in logs](screenshots/02-signin-logs.png)

**Guest account created.** `Add user.` at 16:56:53. UPN is nested inside `ModifiedProperties`, so pulling it out needs `parse_json()` → `mv-expand` → `tostring()`.
...
**Dynamic group membership.** `Add member to group.` fires 58 seconds later, targeting `Operations`. The actor is `ServicePrincipal: Microsoft Approval Management (b412636d-4076-4e18-a804-7f46f2b95c33)`, confirming this was the dynamic-group engine, not a manual action.

```kql
UnifiedAuditLogs_CL
| where Operation == "Add member to group."
| extend Data = parse_json(AuditData)
| project TimeGenerated, UserId, ObjectId, AuditData
```
...
- Alert on short-interval sequences: attribute update → group membership change on the same object.
- Exclude guests from dynamic-group rules that grant meaningful access.
- Gate device-code auth behind Conditional Access, or disable it if unused.
## Indicators
| Type               | Value                                                                 |
|--------------------|------------------------------------------------------------------------|
...
---
*Screenshots in `screenshots/`, numbered in the order referenced above.*
